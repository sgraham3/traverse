        ...
        try:
            for row_idx in range(self.tableWidget.rowCount()):
                ...
                try:
                    ...
                    next_point = (
                        current_point  # Default to no movement, will be updated
                    )
                    ...
                    feat.setAttribute(
                        selected_layer.fields().indexOf("segment_id"), row_idx
                    )
                    # Store the *effective* direction used for drawing this segment
                    feat.setAttribute(
                        selected_layer.fields().indexOf("direction"),
                        self._convert_azimuth_to_bearing_string(
                            segment_tangent_azimuth_deg
                        ),
                    )
                    feat.setAttribute(
                        selected_layer.fields().indexOf("distance"), distance
                    )
                    feat.setAttribute(selected_layer.fields().indexOf("radius"), radius)
                    feat.setAttribute(
                        selected_layer.fields().indexOf("arc_length"), arc_length
                    )
                    ...
                    self.iface.messageBar().pushWarning(
                        "Traverse Plugin",
                        f"Invalid numeric input (Distance, Radius, or Arc Length) in row {row_idx + 1}. Please ensure they are numbers.",
                    )
                    continue
