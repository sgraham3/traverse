...
class traverseDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        self.current_map_tool = (
            None  # To keep track of active map tools for point selection
        )
        ...
        self.actionImport.triggered.connect(
            self.import_data
        )  # Changed actionimport to actionImport
    ...
    def set_start_point(self):
        ...
        if self.iface is None or self.canvas is None:
            self.iface.messageBar().pushCritical(
                "Traverse Plugin",
                "QGIS interface or map canvas not initialized. Please restart QGIS or the plugin.",
            )
            return

        self.iface.messageBar().pushMessage(
            "Traverse Plugin",
            "Click on the map to set the START point of the traverse.",
            level=Qgis.Info,
        )
    ...
    def set_closing_point(self):
        ...
        self.iface.messageBar().pushMessage(
            "Traverse Plugin",
            "Click on the map to set the CLOSING point of the traverse.",
            level=Qgis.Info,
        )
    ...
    def activate_trace_line_tool(self):
        ...
        self.iface.messageBar().pushMessage(
            "Traverse Plugin",
            "Click on the map to define the START of your new traverse segment.",
            level=Qgis.Info,
        )
    ...
    def _handle_trace_point_click(self, clicked_point):
        ...
        if self._first_trace_point is None:
            ...
            self.iface.messageBar().pushMessage(
                "Traverse Plugin",
                "First point set. Now click to define the END of the segment.",
                level=Qgis.Info,
            )
            ...
            # Validate that the segment has non-zero distance
            if distance == 0:
                self.iface.messageBar().pushWarning(
                    "Traverse Plugin",
                    "Segment has zero distance. Please click on a different point.",
                )
                return
            ...
            )  # Radius and Arc Length are 0 for straight lines
    ...
    def _parse_bearing_to_azimuth(self, bearing_str):
        ...
        if quadrant1 == "N" and quadrant2 == "E":
            ...
            quadrant1 == "N" and quadrant2 == "S"
        ):  # Cases like N0-0-0S, this is usually 0 or 180
    ...
    def _convert_azimuth_to_bearing_string(self, azimuth_deg):
        ...
        if minutes >= 60:
            ...
            if degrees == 90:
                if quadrant_prefix == "N" and quadrant_suffix == "E":
                    return "E"
                if quadrant_prefix == "S" and quadrant_suffix == "E":
                    return "S"
                if quadrant_prefix == "S" and quadrant_suffix == "W":
                    return "W"
                if quadrant_prefix == "N" and quadrant_suffix == "W":
                    return "N"
    ...
    def draw_traverse_from_table(self):
        ...
        if selected_layer is None:
            self.iface.messageBar().pushWarning(
                "Traverse Plugin",
                "Please select a layer from the combo box to draw on.",
            )
            return
        ...
        if not (
            ...
            self.iface.messageBar().pushWarning(
                "Traverse Plugin",
                f"Selected layer '{selected_layer.name()}' is not a line layer. Cannot draw traverse lines on it.",
            )
            return
        ...
        if not is_editable_originally:
            ...
            if reply == QtWidgets.QMessageBox.Yes:
                ...
                self.iface.messageBar().pushWarning(
                    "Traverse Plugin",
                    "Layer is not in editing mode. Cannot draw traverse lines.",
                )
                return
        ...
        if self.start_point is None:
            self.iface.messageBar().pushWarning(
                "Traverse Plugin",
                "Please set a START point before drawing traverse lines.",
            )
        ...
        if self.tableWidget.rowCount() == 0:
            self.iface.messageBar().pushWarning(
                "Traverse Plugin", "The traverse table is empty. Add segments to draw."
            )
            # If we started editing for this operation, roll back
            if not is_editable_originally:
                selected_layer.rollBack()
        ...
        if fields_to_add_to_layer.count() > 0:
            if not prov.addAttributes(fields_to_add_to_layer):
                self.iface.messageBar().pushCritical(
                    "Traverse Plugin", "Failed to add required fields to the layer."
                )
            ...
            self.iface.messageBar().pushMessage(
                "Traverse Plugin",
                f"Added missing fields to layer '{selected_layer.name()}'.",
                level=Qgis.Info,
            )
        ...
        try:
            for row_idx in range(self.tableWidget.rowCount()):
                ...
                if not (
                    direction_item and distance_item and distance_item.text().strip()
                ):  # Direction can be empty for tangent
                    self.iface.messageBar().pushWarning(
                        "Traverse Plugin",
                        f"Skipping incomplete row {row_idx + 1} in table for drawing. (Missing Distance or invalid Direction)",
                    )
                    continue
                ...
                if row_idx == 0:  # First segment always uses its own explicit direction
                    ...
                    if direction_str_from_table.startswith("AD:"):
                        ...
                        segment_tangent_azimuth_deg = (
                            last_segment_exit_azimuth + angle_offset
                        ) % 360
                        self.iface.messageBar().pushMessage(
                            "Traverse Plugin",
                ...
                try:
                    distance = (
                        float(distance_item.text())
                        if distance_item and distance_item.text().strip()
                        else 0.0
                    )
                    radius = (
                        float(radius_item.text())
                        if radius_item and radius_item.text().strip()
                        else 0.0
                    )
                    arc_length = (
                        float(arc_length_item.text())
                        if arc_length_item and arc_length_item.text().strip()
                        else 0.0
                    )
                    ...
                    # Check for NC Chord curve (direction does not start with '*' or 'AD:')
                    is_nontangent_chord_curve = (
                        is_curve
                        and not direction_str_from_table.startswith(("*", "AD:"))
                    )

                    if is_nontangent_chord_curve:
                        # --- Nontangent Curve (from Chord) ---
                        try:
                            chord_azimuth_deg = self._parse_bearing_to_azimuth(
                                direction_str_from_table
                            )
                            ...
                            self.iface.messageBar().pushCritical(
                                "Traverse Plugin",
                                f"Failed to calculate NC geometry: {e}",
                            )
                            continue
                    ...
                    if is_nontangent_chord_curve:
                        ...
                        chord_length = distance  # The 'distance' field holds the chord length for NC curves.
                        ...
                        # Angle to find center (perpendicular to chord)
                        center_angle_rad = chord_az_rad + math.copysign(
                            math.pi / 2, -radius
                        )
                        ...
                        for i in range(1, NUM_CURVE_SEGMENTS + 1):
                            ...
                            interp_angle = start_arc_angle + i * step
                        ...
                        # Exit tangent azimuth
                        radial_angle_at_end = math.atan2(
                            next_point.y() - center_y, next_point.x() - center_x
                        )
                        exit_tangent_math_rad = radial_angle_at_end + math.copysign(
                            math.pi / 2, -radius
                        )
                        last_segment_exit_azimuth = (
                            90 - math.degrees(exit_tangent_math_rad) + 360
                        ) % 360
                    ...
                    self.iface.messageBar().pushWarning(
                        "Traverse Plugin", f"Error processing row {row_idx + 1}: {e}"
                    )
                    continue
            ...
            if features_to_add:
                ...
                self.iface.messageBar().pushMessage(
                    "Traverse Plugin",
                    f"Successfully drawn {len(features_to_add)} line segments on layer '{selected_layer.name()}'.",
                    level=Qgis.Info,
                )
            else:
                self.iface.messageBar().pushWarning(
                    "Traverse Plugin", "No valid traverse segments were drawn."
                )

        except Exception as e:
            self.iface.messageBar().pushCritical(
                "Traverse Plugin",
                f"An unexpected error occurred during drawing: {e}. Changes rolled back.",
            )
            ...
            if not is_editable_originally and selected_layer.isEditable():
                ...
                if (
                    self.iface.actionToggleEditing().isChecked()
                ):  # Ensure UI button reflects state
                    self.iface.actionToggleEditing().trigger()
    ...
    def _calculate_nontangent_curve_from_chord(
        ...
        """Calculates the geometry of a nontangent curve defined by its chord."""
        ...
        # For a right turn (+R), center is CCW from chord. For left turn (-R), center is CW.
        center_angle_rad = chord_az_rad + math.copysign(math.pi / 2, radius)

        center_x = mid_x + h * math.sin(center_angle_rad)
        center_y = mid_y + h * math.cos(center_angle_rad)

        # Start and end angles of the main arc
        start_angle_rad = math.atan2(
            start_point.y() - center_y, start_point.x() - center_x
        )
        end_angle_rad = math.atan2(end_chord_y - center_y, end_chord_x - center_x)

        # Generate polyline points for the arc
        polyline = [start_point]

        # Ensure the sweep direction is correct
        delta = end_angle_rad - start_angle_rad
        if radius > 0:  # Right turn, sweep should be negative (CW)
            if delta > 0:
                end_angle_rad -= 2 * math.pi
        else:  # Left turn, sweep should be positive (CCW)
            if delta < 0:
                end_angle_rad += 2 * math.pi

        step = (end_angle_rad - start_angle_rad) / NUM_CURVE_SEGMENTS
        for i in range(1, NUM_CURVE_SEGMENTS + 1):
            angle = start_angle_rad + i * step
            x = center_x + abs(radius) * math.cos(angle)
            y = center_y + abs(radius) * math.sin(angle)
            polyline.append(QgsPointXY(x, y))

        end_point = polyline[-1]

        # Calculate final exit tangent
        radial_at_end = math.atan2(end_point.y() - center_y, end_point.x() - center_x)
        exit_tangent_rad = radial_at_end + math.copysign(math.pi / 2, -radius)
    ...
    def _delete_selected_rows(self):
        ...
        if not selected_rows:
            self.iface.messageBar().pushMessage(
                "Traverse Plugin", "No rows selected to delete.", level=Qgis.Info
            )
            return
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            self.iface.messageBar().pushMessage(
                "Traverse Plugin", "Row deletion cancelled.", level=Qgis.Info
            )
    ...
    def clear_table_and_start_new(self):
        ...
        self.iface.messageBar().pushMessage(
            "Traverse Plugin",
            "Table cleared. Ready for new traverse entry.",
            level=Qgis.Info,
        )
    ...
    def _calculate_chord_length_from_curve(
        self, arc_length: float, radius: float
    ) -> float:
    ...
    def export_data(self):
        ...
        if self.iface is None:
            self.iface.messageBar().pushCritical(
                "Traverse Plugin", "QGIS interface not initialized. Cannot export data."
            )
            return
            ...
            try:
                with open(file_path, "w") as f:
                    ...
                    if self.start_point:
                        ...
                        self.iface.messageBar().pushWarning(
                            "Traverse Plugin",
                            "No start point set. Cannot export full traverse definition without it.",
                        )
                    ...
                    if self.closing_point is None:
                        ...
                        current_calc_azimuth = (
                            None  # Will store exit azimuth of previous segment
                        )
                        ...
                        for row_idx in range(self.tableWidget.rowCount()):
                            ...
                            if not (
                                direction_item
                                and distance_item
                                and distance_item.text().strip()
                            ):  # Direction can be empty for tangent
                                self.iface.messageBar().pushWarning(
                                    "Traverse Plugin",
                                    f"Skipping incomplete row {row_idx + 1} during closing point calculation.",
                                )
                                continue
                            ...
                            if row_idx == 0:
                                ...
                                if direction_str_from_table_calc.startswith("AD:"):
                                    ...
                                    angle_offset_str = direction_str_from_table_calc[
                                        3:
                                    ].strip()
                                    try:
                                        angle_offset = float(angle_offset_str)
                                        ...
                                        continue
                                    segment_tangent_azimuth_deg_calc = (
                                        current_calc_azimuth + angle_offset
                                    ) % 360
                                else:
                                    # Regular explicit direction
                                    try:
                            ...
                            try:
                                ...
                                arc_length = (
                                    ...
                                    if (
                                        arc_length_item
                                        and arc_length_item.text().strip()
                                    )
                                    else 0.0
                                )
                                ...
                                if radius != 0.0 and arc_length != 0.0:
                                    ...
                                    tangent_math_rad_calc = math.radians(
                                        90 - segment_tangent_azimuth_deg_calc
                                    )
                                    center_angle_rad_calc = (
                                        tangent_math_rad_calc
                                        - math.copysign(math.pi / 2, radius)
                                    )

                                    center_x_calc = calculated_closing_point.x() + abs(
                                        radius
                                    ) * math.cos(center_angle_rad_calc)
                                    center_y_calc = calculated_closing_point.y() + abs(
                                        radius
                                    ) * math.sin(center_angle_rad_calc)
                                    center_point_calc = QgsPointXY(
                                        center_x_calc, center_y_calc
                                    )

                                    start_arc_angle_calc = math.atan2(
                                        calculated_closing_point.y() - center_y_calc,
                                        calculated_closing_point.x() - center_x_calc,
                                    )
                                    delta_angle_rad_calc = arc_length / abs(radius)
                                    end_arc_angle_calc = (
                                        start_arc_angle_calc
                                        + math.copysign(delta_angle_rad_calc, -radius)
                                    )
                                    ...
                                    calculated_closing_point = QgsPointXY(
                                        center_x_calc
                                        + abs(radius) * math.cos(end_arc_angle_calc),
                                        center_y_calc
                                        + abs(radius) * math.sin(end_arc_angle_calc),
                                    )

                                    # Calculate exit tangent for the curve
                                    radial_angle_at_end_calc = math.atan2(
                                        calculated_closing_point.y()
                                        - center_point_calc.y(),
                                        calculated_closing_point.x()
                                        - center_point_calc.x(),
                                    )
                                    exit_tangent_math_rad_calc = 0.0
                                    if radius > 0:  # Right turn
                                        exit_tangent_math_rad_calc = (
                                            radial_angle_at_end_calc + math.pi / 2
                                        )
                                    else:  # Left turn
                                        exit_tangent_math_rad_calc = (
                                            radial_angle_at_end_calc - math.pi / 2
                                    ...
                                    current_calc_azimuth = (
                                        90 - math.degrees(exit_tangent_math_rad_calc)
                                    ) % 360
                                    ...
                                    azimuth_rad_calc = math.radians(
                                        segment_tangent_azimuth_deg_calc
                                    )
                                    dx = distance * math.sin(azimuth_rad_calc)
                                    dy = distance * math.cos(azimuth_rad_calc)
                                    calculated_closing_point = QgsPointXY(
                                        calculated_closing_point.x() + dx,
                                        calculated_closing_point.y() + dy,
                                    )
                                    current_calc_azimuth = (
                                        segment_tangent_azimuth_deg_calc
                                    )
                        ...
                        if calculated_closing_point:
                            f.write(
                                f"EP {calculated_closing_point.x():.6f} {calculated_closing_point.y():.6f}\n"
                            )
                            self.iface.messageBar().pushMessage(
                                "Traverse Plugin",
                                "Closing point calculated from traverse segments and exported.",
                                level=Qgis.Info,
                            )
                        else:
                            self.iface.messageBar().pushWarning(
                                "Traverse Plugin",
                                "Could not calculate closing point from traverse segments. Check table data.",
                            )
                    else:
                    ...
                    for row_idx in range(self.tableWidget.rowCount()):
                        ...
                        if (
                            ...
                            if radius != 0.0 and arc_length != 0.0:
                                ...
                                if (
                                    direction_str_from_table == "*"
                                    or not direction_str_from_table
                                ):
                                    # Tangent curve (TC)
                                    ...
                                    try:
                                        ...
                                        cleaned_direction_str = (
                                            direction_str_from_table.replace(
                                                "°", ""
                                            ).strip()
                                        )
                                        azimuth_deg_for_export = float(
                                            cleaned_direction_str
                                        )
                                    except ValueError:
                                        try:
                                ...
                                if direction_str_from_table.startswith("AD:"):
                                    ...
                                    angle_offset_str = direction_str_from_table[
                                        3:
                                    ].strip()
                                    try:
                                        angle_offset = float(angle_offset_str)
                                    ...
                                    f.write(f"AD {angle_offset:.2f} {distance:.6f}\n")
                                else:
                ...
                self.iface.messageBar().pushMessage(
                    "Traverse Plugin",
                    f"Traverse data successfully exported to {os.path.basename(file_path)}.",
                    level=Qgis.Info,
                )
            except Exception as e:
                self.iface.messageBar().pushCritical(
                    "Traverse Plugin", f"An error occurred during export: {e}"
                )
    ...
    def on_layer_changed(self, layer):
        ...
        if layer:
            self.iface.messageBar().pushMessage(
                "Traverse Plugin",
                f"Selected layer: {layer.name()} (ID: {layer.id()})",
                level=Qgis.Info,
            )
        else:
            self.iface.messageBar().pushMessage(
                "Traverse Plugin",
                "No layer selected in the combo box.",
                level=Qgis.Info,
            )
    ...
    def add_traverse_segment(self, direction, distance, radius, arc_length):
        ...
        self.tableWidget.setItem(
            row_count, 0, QtWidgets.QTableWidgetItem(str(direction))
        )
        self.tableWidget.setItem(
            row_count, 1, QtWidgets.QTableWidgetItem(f"{distance:.3f}")
        )
        self.tableWidget.setItem(
            row_count, 2, QtWidgets.QTableWidgetItem(f"{radius:.3f}")
        )
        self.tableWidget.setItem(
            row_count, 3, QtWidgets.QTableWidgetItem(f"{arc_length:.3f}")
        )
    ...
    def _add_single_empty_row(self):
        ...
        self.tableWidget.insertRow(row_count)
        self.tableWidget.setItem(
            row_count, 0, QtWidgets.QTableWidgetItem("")
        )  # Empty direction
        self.tableWidget.setItem(
            row_count, 1, QtWidgets.QTableWidgetItem("0.000")
        )  # Default distance
        self.tableWidget.setItem(
            row_count, 2, QtWidgets.QTableWidgetItem("0.000")
        )  # Default radius
        self.tableWidget.setItem(
            row_count, 3, QtWidgets.QTableWidgetItem("0.000")
        )  # Default arc_length
        self.iface.messageBar().pushMessage(
            "Traverse Plugin", "Added new empty row to the table.", level=Qgis.Info
        )
